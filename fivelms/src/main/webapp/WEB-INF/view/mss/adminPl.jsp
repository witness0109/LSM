...
<script type="text/javascript">
	...
	function fSelectBox(rm_seq){
		...
		var resultCallback = function(data){
			fSelectBoxResult(data);
		};
		callAjax("/mss/SelectBox.do", "post", "json", true, param,
				resultCallback);
	}
	function fSelectBoxResult(data){
	
		
		console.log(data);
		var selectdata = data.rList;
		rm = selectdata.rm_seq;
		console.log(selectdata);
		
		$('#rm_seq').children().remove();
		
		for( var count in selectdata) {
			
			var option = $("<option value='" + selectdata[count].rm_seq  + "'>"+selectdata[count].rm_seq +"</option>");
            
			$('#rm_seq').append(option);
	...
	function fSelectPlistResult(data) {
		...
		if (data.result == "SUCCESS") {
			...
			fInitFormaPlist(data.plist, data.loginId, data.indate);
		}

	}
	/*강의 목록 폼 모델 초기화*/
	function fInitFormaPlist(object, object2, object3) {
		alert(object);
		console.log(object);
						...
						<tbody>
							...
							<tr>
								...
								<td colspan="3">
									...
									<select name="rm_seq" id="rm_seq">
								
									</select>
							
								<!-- 	<input type="text" class="inputTxt p100" name="rm_seq" id="rm_seq" /> -->
								</td>
							</tr>
							<tr>
